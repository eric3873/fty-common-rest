...
namespace email {
    ...
    void x_headers(zhash_t* headers)
    {
        ...
        zhash_insert(
            headers, "X-Eaton-IPC-hardware-catalog-number", s_getenv("HARDWARE_CATALOG_NUMBER", "unknown"));
        zhash_insert(
            headers, "X-Eaton-IPC-hardware-spec-revision", s_getenv("HARDWARE_SPEC_REVISION", "unknown"));
        zhash_insert(
            headers, "X-Eaton-IPC-hardware-serial-number", s_getenv("HARDWARE_SERIAL_NUMBER", "unknown"));
...
void fty_common_rest_utils_web_test(bool verbose)
{
...

    {
        ...
        assert(x.compare(std::string("\"") + std::to_string(var_uint64_t) +
                         "\" : " + "\"*const char with a '\\\"' quote and newline \\\\n '\\\\\\\"'\"") == 0);
